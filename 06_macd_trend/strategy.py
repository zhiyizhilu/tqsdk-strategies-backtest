...
# -*- coding: utf-8 -*-
"""
MACD趋势跟踪策略 (MACD Trend Strategy)
====================================================

策略逻辑：
    - 使用MACD指标（DIF、DEA）的交叉信号判断趋势方向
    - 金叉（DIF从下方向上穿越DEA）：目标仓位设为 +VOLUME（做多）
    - 死叉（DIF从上方向下穿越DEA）：目标仓位设为 -VOLUME（做空）
    - 使用 TargetPosTask 管理持仓，无需手动处理追单、撤单、部分成交等细节

适用品种：
    趋势性较强的品种，如原油（INE.sc）、黄金（SHFE.au）、铜（SHFE.cu）、股指（CFFEX.IF/IC/IM）等

风险提示：
    - MACD是滞后指标，震荡行情中容易产生频繁的假信号（亏损）
    - 建议结合成交量、波动率等过滤器使用
    - 本代码仅供学习参考，不构成任何投资建议

参数说明：
    SYMBOL        : 交易合约代码，格式为 "交易所.合约代码"
    FAST_PERIOD   : 快线EMA周期（K线根数）
    SLOW_PERIOD   : 慢线EMA周期（K线根数）
    SIGNAL_PERIOD : 信号线DEA周期（K线根数）
...
class MACDTrendStrategy:
    ...
    使用方式：
        strategy = MACDTrendStrategy(api, logger, symbol="SHFE.cu2506", fast_period=12, slow_period=26, signal_period=9)
    ...
    def __init__(
        ...
        self.fast_period = fast_period or self.DEFAULT_FAST_PERIOD
        self.slow_period = slow_period or self.DEFAULT_SLOW_PERIOD
        ...
        data_length = max(self.slow_period, self.signal_period) + 50
        ...
        self.logger.info(
            f"[策略初始化] MACD趋势策略 | 合约: {self.symbol} | "
            f"快线: {self.fast_period} | 慢线: {self.slow_period} | 信号线: {self.signal_period} | "
    ...
    def on_kline_update(self) -> int:
        ...
        计算MACD指标（DIF、DEA），判断是否出现金叉或死叉信号，并返回相应的目标仓位
        ...
        is_golden = bool(crossup(dif, dea).iloc[-1])
        is_death = bool(crossdown(dif, dea).iloc[-1])

        target_volume = 0

        if is_golden:
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(f">>> 金叉！DIF上穿DEA | 目标仓位: +{dynamic_vol}（做多）")
            target_volume = dynamic_vol
            self.last_signal = "golden"

        elif is_death:
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(f">>> 死叉！DIF下穿DEA | 目标仓位: -{dynamic_vol}（做空）")
            target_volume = -dynamic_vol
...
def create_strategy(
    ...
    Args:
        ...
        fast_period: 快线EMA周期
        slow_period: 慢线EMA周期
        signal_period: 信号线DEA周期
        kline_dur: K线周期（秒）
        volume: 持仓手数

    Returns:
        MACDTrendStrategy: 策略实例
    """
