...
<title>MACD趋势策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 MACD趋势策略 — 多品种回测分析报告</h1>
  <div class="subtitle">MACD Trend Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 快线EMA{fast_period} | 慢线EMA{slow_period} | 信号DEA{signal_period}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由MACD趋势策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
