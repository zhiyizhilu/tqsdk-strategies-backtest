...
class RSIMeanReversionStrategy:
    ...
    使用方式：
        strategy = RSIMeanReversionStrategy(api, logger, symbol="CZCE.MA501", rsi_period=14, overbought=70, oversold=30)
        strategy.run()  # 阻塞运行，直到策略结束
    """

    # 默认交易合约：郑商所甲醇501合约
    DEFAULT_SYMBOL = "CZCE.MA501"
    # 默认RSI计算周期：14根K线
    ...
    def __init__(
        ...
        Args:
            ...
            symbol: 交易合约代码，格式为 "交易所.合约代码"，或连续合约如 "KQ.m@CZCE.MA"
            rsi_period: RSI计算周期（K线根数）
            overbought: 超买阈值
            oversold: 超卖阈值
            kline_dur: K线周期（秒），60=1分钟K线，900=15分钟K线
        ...
        self.rsi_period = rsi_period or self.DEFAULT_RSI_PERIOD  # 使用默认RSI周期如果未指定
        self.overbought = overbought or self.DEFAULT_OVERBOUGHT  # 使用默认超买阈值如果未指定
        self.oversold = oversold or self.DEFAULT_OVERSOLD  # 使用默认超卖阈值如果未指定
        ...
        # 获取K线数据，数据长度为RSI周期的3倍，确保有足够的数据计算RSI
        self.klines = api.get_kline_serial(
            self.symbol, self.kline_dur, data_length=self.rsi_period * 3
        )
        ...
        # 记录策略初始化信息
        self.logger.info(
            f"[策略初始化] RSI均值回归策略 | 合约: {self.symbol} | "
            f"RSI周期: {self.rsi_period} | 超买: {self.overbought} | 超卖: {self.oversold} | "
    ...
    def calc_rsi(self, close_series: pd.Series, period: int) -> pd.Series:
        """计算 RSI（Wilder 平滑方法）"""
    ...
    def on_kline_update(self) -> int:
        ...
        计算RSI值，判断是否出现超买超卖信号，并返回相应的目标仓位

        Returns:
            int: 目标仓位（正数=多头，负数=空头，0=空仓）
        """
        # 计算RSI
        rsi_series = self.calc_rsi(self.klines.close, self.rsi_period)
        ...
        if rsi < self.oversold:
            ...
            self.logger.info(f">>> RSI={rsi:.1f} 超卖，目标仓位: +{dynamic_vol}（做多）")
        ...
        # 处理超买信号：做空
            ...
            # 使用动态计算的手数（基于总资产×保证金比例）
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(f">>> RSI={rsi:.1f} 超买，目标仓位: -{dynamic_vol}（做空）")
            target_volume = -dynamic_vol  # 设置为空头仓位
            self.last_signal = "overbought"

        # 处理回归中性区信号：平仓
        elif 40 < rsi < 60:
            self.logger.info(f">>> RSI={rsi:.1f} 回归中性，目标仓位: 0（平仓）")
            target_volume = 0  # 设置为空仓
        ...
        while True:
            ...
            if target_volume != 0 or (target_volume == 0 and self.current_target_volume != 0):
                ...
                if self._use_insert_order:
                    ...
                    try:
                        # 计算需要调整的仓位
                        delta_volume = target_volume - self._current_position
                        if delta_volume == 0:
                            continue
                        
                        direction = "BUY" if delta_volume > 0 else "SELL"
                        offset = "OPEN" if abs(self._current_position) == 0 else "CLOSE"
                        ...
                        self.logger.info(f"[交易-insert] 日期: {trade_date} | 合约: {self.current_trading_symbol} | 方向:{direction} | 手数:{abs(delta_volume)}")
...
def create_strategy(
    ...
    提供一个便捷的方式创建RSI均值回归策略实例，使用默认参数或自定义参数

    Args:
        api: TqApi 实例，用于与期货交易系统交互
        logger: 日志记录器，用于记录策略运行信息
        symbol: 交易合约代码，默认值为 "CZCE.MA501"（郑商所甲醇501合约）
        rsi_period: RSI计算周期，默认值为 14
        overbought: 超买阈值，默认值为 70
        oversold: 超卖阈值，默认值为 30
        kline_dur: K线周期（秒），默认值为 900（15分钟K线）
        volume: 持仓手数，默认值为 1

    Returns:
        RSIMeanReversionStrategy: 策略实例
    """
