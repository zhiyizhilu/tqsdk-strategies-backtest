...
def render_html(results, stats, chart_data, config):
    ...
    init_af = config.get("init_af", 0.02)
    step = config.get("step", 0.02)
    max_af = config.get("max_af", 0.20)
...
<title>抛物线SAR策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 抛物线SAR策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Parabolic SAR Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 AF初始：{init_af} | 步长：{step} | 最大：{max_af}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由抛物线SAR策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
