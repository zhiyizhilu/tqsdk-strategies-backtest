...
def run_backtest(symbol: str, config: dict, tq_account: str, tq_password: str) -> dict:
    ...
    print(f"动量周期: {momentum_n} | 开仓阈值: {threshold}% | 平仓阈值: {exit_threshold}%")
    print(f"均线过滤: {'开启' if use_ma_filter else '关闭'} | 均线周期: MA{filter_n}")
...
def main():
    parser = argparse.ArgumentParser(description='动量突破策略多品种回测')
