...
def render_html(results, stats, chart_data, config):
    ...
    momentum_n = config.get("momentum_n", "N/A")
    threshold = config.get("threshold", "N/A")
    exit_threshold = config.get("exit_threshold", "N/A")
    filter_n = config.get("filter_n", "N/A")
    use_ma_filter = config.get("use_ma_filter", True)
    ...
    ma_filter_text = f"MA{filter_n} 过滤{'开启' if use_ma_filter else '关闭'}"
...
<div class="header">
  <h1>📊 动量突破策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Momentum Breakout Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 动量周期：{momentum_n} | 开仓阈值：{threshold}% | 平仓阈值：{exit_threshold}%</span>
    <span>🔍 {ma_filter_text}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由动量突破策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
