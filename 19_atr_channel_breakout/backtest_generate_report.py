...
def render_html(results, stats, chart_data, config):
    ...
    atr_multi = config.get("atr_multi", 2.5)
...
<title>ATR通道突破策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 ATR通道突破策略 — 多品种回测分析报告</h1>
  <div class="subtitle">ATR Channel Breakout Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 ATR周期：{atr_period} | ATR倍数：{atr_multi} | 中轨：MA{ma_period}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由ATR通道突破策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
