...
<div class="header">
  <h1>🐢 海龟交易策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Turtle Trading Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 入场N1={n1} | 出场N2={n2} | ATR={atr_period}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由海龟交易策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
