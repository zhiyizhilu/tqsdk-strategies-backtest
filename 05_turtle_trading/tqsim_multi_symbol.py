...
def run_backtest(symbol: str, config: dict, tq_account: str, tq_password: str) -> dict:
    from strategy import TurtleTradingStrategy
    ...
    risk_ratio = config.get("risk_ratio", 0.01)
    contract_multiplier = config.get("contract_multiplier", None)
    ...
    print(f"入场通道: N1={n1} | 出场通道: N2={n2} | ATR周期: {atr_period}")
    print(f"风险比: {risk_ratio} | 最大手数: {max_volume} | 初始资金: {initial_balance:,.0f}元")
    if margin_ratio is not None:
        print(f"仓位模式: 动态（总资产×{margin_ratio:.0%}保证金）")
    else:
        print(f"仓位模式: ATR波动率仓位")
...
def main():
    parser = argparse.ArgumentParser(description='海龟交易策略多品种回测')
