...
def render_html(results, stats, chart_data, config):
    ...
    ema_period = config.get("ema_period", "N/A")
    atr_period = config.get("atr_period", "N/A")
    atr_mult = config.get("atr_mult", "N/A")
    kline_dur = config.get("kline_dur", 900)
...
<div class="header">
  <h1>📊 肯特纳通道策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Keltner Channel Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 EMA：{ema_period} | ATR：{atr_period}×{atr_mult}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由肯特纳通道策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
