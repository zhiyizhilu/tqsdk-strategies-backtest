...
def run_backtest(symbol: str, config: dict, tq_account: str, tq_password: str) -> dict:
    ...
    breakout_n = config.get("breakout_n", 20)
    exit_n = config.get("exit_n", 10)
    vol_ma_n = config.get("vol_ma_n", 20)
    vol_multiplier = config.get("vol_multiplier", 1.5)
    kline_dur = config.get("kline_dur", 86400)
    volume = config.get("volume", 1)
    data_length = config.get("data_length", 150)
...
def main():
    parser = argparse.ArgumentParser(description='量价趋势策略多品种回测')
