...
def render_html(results, stats, chart_data, config):
    ...
    breakout_n = config.get("breakout_n", "N/A")
    exit_n = config.get("exit_n", "N/A")
    vol_ma_n = config.get("vol_ma_n", "N/A")
    vol_multiplier = config.get("vol_multiplier", "N/A")
    kline_dur = config.get("kline_dur", 86400)
...
<div class="header">
  <h1>📊 量价趋势策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Volume Price Trend Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 突破周期：{breakout_n} | 平仓周期：{exit_n} | 量线周期：{vol_ma_n} | 放量倍数：{vol_multiplier}x</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由量价趋势策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
