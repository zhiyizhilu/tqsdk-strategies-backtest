...
def render_html(results, stats, chart_data, config):
    ...
    aroon_period = config.get("aroon_period", 25)
    aroon_thresh = config.get("aroon_thresh", 70)
...
<title>Aroon趋势策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 Aroon趋势策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Aroon Trend Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 Aroon周期：{aroon_period} | 阈值：{aroon_thresh}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由Aroon趋势策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
