...
# -*- coding: utf-8 -*-
"""
Aroon指标趋势策略 (Aroon Trend Strategy)
====================================================

策略逻辑：
    - 使用Aroon Up和Aroon Down的交叉信号判断趋势方向
    - Aroon Up上穿Aroon Down且Aroon Up超过阈值：目标仓位设为 +VOLUME（做多）
    - Aroon Down上穿Aroon Up且Aroon Down超过阈值：目标仓位设为 -VOLUME（做空）
    - 使用 TargetPosTask 管理持仓，无需手动处理追单、撤单、部分成交等细节

适用品种：
    趋势性较强的品种，如原油（INE.sc）、铜（SHFE.cu）、黄金（SHFE.au）、螺纹钢（SHFE.rb）

风险提示：
    - Aroon策略在震荡行情中容易产生频繁的假信号（亏损）
    - 建议结合成交量、波动率等过滤器使用
    - 本代码仅供学习参考，不构成任何投资建议
...
class AroonTrendStrategy:
    """
    Aroon趋势策略类

    使用方式：
        strategy = AroonTrendStrategy(api, logger, symbol="SHFE.rb2605", aroon_period=25, aroon_thresh=70)
        strategy.run()  # 阻塞运行，直到策略结束
    """

    DEFAULT_SYMBOL = "SHFE.rb2605"
    DEFAULT_AROON_PERIOD = 25
    DEFAULT_AROON_THRESH = 70
    ...
    def __init__(
        ...
        logger,
        symbol: str = None,
        aroon_period: int = None,
        aroon_thresh: int = None,
        kline_dur: int = None,
        volume: int = None,
        ...
        Args:
            ...
            logger: 日志记录器
            symbol: 交易合约代码
            aroon_period: Aroon计算周期（K线根数）
            aroon_thresh: Aroon强势阈值（0~100）
            kline_dur: K线周期（秒）
            volume: 固定持仓手数
            data_length: 历史K线数量
        ...
        self.aroon_period = aroon_period or self.DEFAULT_AROON_PERIOD
        self.aroon_thresh = aroon_thresh or self.DEFAULT_AROON_THRESH
        ...
        self.logger.info(
            f"[策略初始化] Aroon趋势策略 | 合约: {self.symbol} | "
            f"Aroon周期: {self.aroon_period} | 阈值: {self.aroon_thresh} | "
    ...
    def on_kline_update(self) -> int:
        ...
        计算Aroon Up和Aroon Down，判断交叉信号，返回目标仓位
        ...
        aroon_up, aroon_down = calc_aroon(high, low, self.aroon_period)

        aroon_up_s = pd.Series(aroon_up, name="aroon_up")
        aroon_down_s = pd.Series(aroon_down, name="aroon_down")

        cross_up_signal = crossup(aroon_up_s, aroon_down_s)
        cross_down_signal = crossup(aroon_down_s, aroon_up_s)

        last_aroon_up = aroon_up[-1]
        last_aroon_down = aroon_down[-1]
        last_cross_up = bool(cross_up_signal.iloc[-1])
        last_cross_down = bool(cross_down_signal.iloc[-1])

        if np.isnan(last_aroon_up) or np.isnan(last_aroon_down):
            return 0

        target_volume = 0

        if last_cross_up and last_aroon_up > self.aroon_thresh:
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(
                f">>> Aroon Up上穿Down！AroonUp={last_aroon_up:.1f} | 目标仓位: +{dynamic_vol}（做多）"
            )
            target_volume = dynamic_vol
            self.last_signal = "aroon_up_cross"

        elif last_cross_down and last_aroon_down > self.aroon_thresh:
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(
                f">>> Aroon Down上穿Up！AroonDown={last_aroon_down:.1f} | 目标仓位: -{dynamic_vol}（做空）"
            )
            target_volume = -dynamic_vol
            self.last_signal = "aroon_down_cross"
...
def create_strategy(
    ...
    Args:
        ...
        logger: 日志记录器
        symbol: 交易合约代码
        aroon_period: Aroon计算周期
        aroon_thresh: Aroon强势阈值
        kline_dur: K线周期（秒）
        volume: 持仓手数

    Returns:
        AroonTrendStrategy: 策略实例
    """
