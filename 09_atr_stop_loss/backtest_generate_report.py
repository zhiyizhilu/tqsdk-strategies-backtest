...
def render_html(results, stats, chart_data, config):
    ...
    ma_fast = config.get("ma_fast", 10)
    ma_slow = config.get("ma_slow", 30)
...
<div class="header">
  <h1>📊 ATR止损策略 — 多品种回测分析报告</h1>
  <div class="subtitle">ATR Trailing Stop Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 MA快：{ma_fast} | MA慢：{ma_slow} | ATR({atr_n})×{atr_multiplier}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由ATR止损策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
