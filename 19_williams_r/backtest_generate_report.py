...
def render_html(results, stats, chart_data, config):
    ...
    wr_period = config.get("wr_period", 14)
    overbought_line = config.get("overbought_line", -20)
    oversold_line = config.get("oversold_line", -80)
...
<div class="header">
  <h1>📊 威廉指标策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Williams %R Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 %R周期：{wr_period} | 超买：{overbought_line} | 超卖：{oversold_line} | MA过滤：{ma_period}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由威廉指标策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
