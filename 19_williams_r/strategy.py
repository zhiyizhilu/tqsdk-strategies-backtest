...
策略逻辑：
    - 使用威廉指标（Williams %R）判断超买超卖状态
    - %R从超卖区（< -80）向上穿越超卖线 → 做多
    - %R从超买区（> -20）向下穿越超买线 → 做空
    - %R进入超买区 → 止盈平多仓
    - %R进入超卖区 → 止盈平空仓
    - 可选MA均线趋势过滤：均线向上时只做多，均线向下时只做空
    - 使用 TargetPosTask 管理持仓，无需手动处理追单、撤单、部分成交等细节

适用品种：
    有明显日内波动规律的品种，如铜（SHFE.cu）、黄金（SHFE.au）、股指（CFFEX.IF）
...
def calc_williams_r(high, low, close, period):
    ...
    """
    highest_high = hhv(high, period)
    lowest_low = llv(low, period)
...
class WilliamsRStrategy:
    ...
    使用方式：
        strategy = WilliamsRStrategy(api, logger, symbol="SHFE.cu2501", wr_period=14)
        strategy.run()  # 阻塞运行，直到策略结束
    """

    DEFAULT_SYMBOL = "SHFE.cu2505"
    DEFAULT_WR_PERIOD = 14
    DEFAULT_OVERBOUGHT_LINE = -20
    DEFAULT_OVERSOLD_LINE = -80
    ...
    def __init__(
        ...
        symbol: str = None,
        wr_period: int = None,
        overbought_line: float = None,
        oversold_line: float = None,
        ma_period: int = None,
        kline_dur: int = None,
        volume: int = None,
        ...
        Args:
            ...
            wr_period: 威廉指标计算周期（K线根数）
            overbought_line: 超买线（%R > 此值为超买）
            oversold_line: 超卖线（%R < 此值为超卖）
            ma_period: 趋势过滤均线周期（0则不使用均线过滤）
            kline_dur: K线周期（秒），60=1分钟K线，300=5分钟K线
        ...
        self.wr_period = wr_period or self.DEFAULT_WR_PERIOD
        self.overbought_line = overbought_line if overbought_line is not None else self.DEFAULT_OVERBOUGHT_LINE
        self.oversold_line = oversold_line if oversold_line is not None else self.DEFAULT_OVERSOLD_LINE
        self.ma_period = ma_period if ma_period is not None else self.DEFAULT_MA_PERIOD
        ...
        self.logger.info(
            f"[策略初始化] 威廉指标策略 | 合约: {self.symbol} | "
            f"%R周期: {self.wr_period} | 超买线: {self.overbought_line} | 超卖线: {self.oversold_line} | "
            f"MA过滤周期: {self.ma_period} | "
    ...
    def on_kline_update(self) -> int:
        ...
        计算威廉指标%R，判断超买超卖穿越信号，并返回相应的目标仓位
        ...
        if self.ma_period > 0:
            ma_line = ma(close, self.ma_period)
            ma_now = ma_line.iloc[-1]
            ma_prev = ma_line.iloc[-2]
            trend_up = ma_now > ma_prev
        else:
            trend_up = True

        cross_out_oversold = (wr_prev < self.oversold_line) and (wr_now >= self.oversold_line)
        cross_out_overbought = (wr_prev > self.overbought_line) and (wr_now <= self.overbought_line)
        enter_overbought = (wr_prev <= self.overbought_line) and (wr_now > self.overbought_line)
        enter_oversold = (wr_prev >= self.oversold_line) and (wr_now < self.oversold_line)

        target_volume = 0

        if cross_out_oversold and trend_up:
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(f">>> %R上穿超卖线！目标仓位: +{dynamic_vol}（做多）| %R={wr_now:.2f}")
            target_volume = dynamic_vol
            self.last_signal = "long"

        elif cross_out_overbought and not trend_up:
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(f">>> %R下穿超买线！目标仓位: -{dynamic_vol}（做空）| %R={wr_now:.2f}")
            target_volume = -dynamic_vol
            self.last_signal = "short"

        elif enter_overbought:
            self.logger.info(f">>> %R进入超买区，平多仓 | %R={wr_now:.2f}")
            target_volume = 0
            self.last_signal = "close_long"

        elif enter_oversold:
            self.logger.info(f">>> %R进入超卖区，平空仓 | %R={wr_now:.2f}")
            target_volume = 0
...
def create_strategy(
    api: TqApi,
    logger,
    symbol: str = "SHFE.cu2505",
    wr_period: int = 14,
    overbought_line: float = -20,
    oversold_line: float = -80,
    ma_period: int = 20,
    kline_dur: int = 5 * 60,
    volume: int = 1,
) -> WilliamsRStrategy:
