...
def render_html(results, stats, chart_data, config):
    ...
    n_period = config.get("n_period", "N/A")
    k_times = config.get("k_times", "N/A")
    ...
    min_band_width = config.get("min_band_width", "N/A")
...
<div class="header">
  <h1>📊 布林带突破策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Bollinger Bands Breakout Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 周期: {n_period} | 倍数: {k_times} | 最小带宽: {min_band_width}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由布林带突破策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
...
def main():
    print("=" * 60)
    print("布林带突破策略 - 回测报告生成器")
    print("=" * 60)

    if not os.path.exists(CSV_FILE):
        print(f"[错误] 未找到回测结果文件: {CSV_FILE}")
        print("请先运行 multi_symbol_backtest.py --all 完成回测")
        return
