...
使用方法：
    ...
    4. 可选：运行特定品种：python tqsim_multi_symbol.py --symbol KQ.m@CFFEX.IF
"""
...
def main():
    parser = argparse.ArgumentParser(description='开盘区间突破策略多品种回测')
    parser.add_argument('--symbol', type=str, help='指定测试的品种代码，如 KQ.m@CFFEX.IF')
    parser.add_argument('--all', action='store_true', help='测试所有品种')
    args = parser.parse_args()

    print("="*60)
    print("开盘区间突破策略 - 多品种回测")
    print("="*60)
