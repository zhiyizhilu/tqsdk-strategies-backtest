...
# -*- coding: utf-8 -*-
"""
开盘区间突破策略多品种回测结果分析报告生成器
=========================================

读取 backtest_result.csv，生成美观的 HTML 分析报告。
"""
...
def generate_chart_data(results):
    ...
    bar_colors = ["#c0392b" if v >= 0 else "#27ae60" for v in bar_values]
...
def render_html(results, stats, chart_data, config):
    ...
    open_range_minutes = config.get("open_range_minutes", 30)
    open_hour = config.get("open_hour", 9)
    open_min = config.get("open_min", 30)
    close_hour = config.get("close_hour", 14)
    close_min = config.get("close_min", 45)
    stop_loss_multiplier = config.get("stop_loss_multiplier", 1.0)
    kline_dur = config.get("kline_dur", 60)
...
<title>开盘区间突破策略 - 多品种回测分析报告</title>
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .header .meta {{ margin-top: 20px; display: flex; gap: 32px; font-size: 13px; opacity: 0.9; flex-wrap: wrap; }}
...
<div class="header">
  <h1>📊 开盘区间突破策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Opening Range Breakout Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 开盘区间：{open_range_minutes}分钟（{open_hour:02d}:{open_min:02d}~{open_hour + (open_min + open_range_minutes) // 60:02d}:{(open_min + open_range_minutes) % 60:02d}）</span>
    <span>🕐 收盘平仓：{close_hour:02d}:{close_min:02d}</span>
    <span>🛡️ 止损倍数：{stop_loss_multiplier}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由开盘区间突破策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
...
def main():
    print("=" * 60)
    print("开盘区间突破策略 - 回测报告生成器")
    print("=" * 60)

    if not os.path.exists(CSV_FILE):
        print(f"[错误] 未找到回测结果文件: {CSV_FILE}")
        print("请先运行 tqsim_multi_symbol.py --all 完成回测")
        return
