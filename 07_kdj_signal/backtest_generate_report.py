...
def render_html(results, stats, chart_data, config):
    ...
    kdj_n = config.get("kdj_n", 9)
    kdj_m1 = config.get("kdj_m1", 3)
    kdj_m2 = config.get("kdj_m2", 3)
    overbuy = config.get("overbuy", 80)
    oversell = config.get("oversell", 20)
...
<div class="header">
  <h1>📊 KDJ信号策略 — 多品种回测分析报告</h1>
  <div class="subtitle">KDJ Signal Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 KDJ：N={kdj_n} M1={kdj_m1} M2={kdj_m2} | 超买={overbuy} 超卖={oversell}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由KDJ信号策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
