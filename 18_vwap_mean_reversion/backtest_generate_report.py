...
def render_html(results, stats, chart_data, config):
    ...
    deviation = config.get("deviation", 0.5)
...
<title>VWAP均值回归策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 VWAP均值回归策略 — 多品种回测分析报告</h1>
  <div class="subtitle">VWAP Mean Reversion Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 VWAP周期：{vwap_period} | 偏离阈值：{deviation}%</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由VWAP均值回归策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
