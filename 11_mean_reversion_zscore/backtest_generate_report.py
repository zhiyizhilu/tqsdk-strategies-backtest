...
def render_html(results, stats, chart_data, config):
    ...
    zscore_n = config.get("zscore_n", "N/A")
    entry_z = config.get("entry_z", "N/A")
    exit_z = config.get("exit_z", "N/A")
    max_hold_bars = config.get("max_hold_bars", "N/A")
    kline_dur = config.get("kline_dur", 1800)
...
<title>Z-Score均值回归策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 Z-Score均值回归策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Mean Reversion Z-Score Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 回望N：{zscore_n} | 开仓Z：{entry_z} | 平仓Z：{exit_z} | 最大持仓：{max_hold_bars}根</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由Z-Score均值回归策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
