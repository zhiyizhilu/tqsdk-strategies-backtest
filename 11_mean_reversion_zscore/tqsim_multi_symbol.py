...
# -*- coding: utf-8 -*-
"""
Z-Score均值回归策略 - 多品种回测脚本
=====================================

功能：
    - 循环测试多个品种的Z-Score均值回归策略
    - 汇总各品种的收益情况
    - 将结果导出到CSV文件
...
使用方法：
    ...
    4. 可选：运行特定品种：python tqsim_multi_symbol.py --symbol KQ.m@DCE.m
"""
...
def run_backtest(symbol: str, config: dict, tq_account: str, tq_password: str) -> dict:
    ...
    zscore_n = config.get("zscore_n", 20)
    entry_z = config.get("entry_z", 2.0)
    exit_z = config.get("exit_z", 0.5)
    max_hold_bars = config.get("max_hold_bars", 10)
    ...
    print(f"回望N: {zscore_n} | 开仓Z: {entry_z} | 平仓Z: {exit_z} | 最大持仓: {max_hold_bars}根")
...
def main():
    parser = argparse.ArgumentParser(description='Z-Score均值回归策略多品种回测')
    parser.add_argument('--symbol', type=str, help='指定测试的品种代码，如 KQ.m@DCE.m')
    parser.add_argument('--all', action='store_true', help='测试所有品种')
    args = parser.parse_args()

    print("="*60)
    print("Z-Score均值回归策略 - 多品种回测")
    print("="*60)
