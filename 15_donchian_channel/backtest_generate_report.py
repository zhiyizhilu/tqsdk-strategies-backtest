...
<div class="header">
  <h1>📊 唐奇安通道策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Donchian Channel Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 入场N：{n_enter} | 出场N：{n_exit}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由唐奇安通道策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
