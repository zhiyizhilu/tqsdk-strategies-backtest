...
def render_html(results, stats, chart_data, config):
    ...
    day_ma_n = config.get("day_ma_n", 20)
    hour_ma_n = config.get("hour_ma_n", 20)
    min15_fast_n = config.get("min15_fast_n", 5)
    min15_slow_n = config.get("min15_slow_n", 20)
...
<title>多周期均线共振策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 多周期均线共振策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Multi-Timeframe Moving Average Confluence Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>📈 日MA{day_ma_n} + 时MA{hour_ma_n} + 15m MA{min15_fast_n}/{min15_slow_n}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由多周期均线共振策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
