...
def render_html(results, stats, chart_data, config):
    ...
    short_period = config.get("short_period", "N/A")
    long_period = config.get("long_period", "N/A")
...
<div class="header">
  <h1>📊 双均线策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Double Moving Average Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 短均线：MA{short_period} | 长均线：MA{long_period}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由双均线策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
