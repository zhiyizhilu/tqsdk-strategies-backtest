...
<div class="header">
  <h1>📊 一目均衡图策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Ichimoku Cloud Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 转换线：{tenkan_period} | 基准线：{kijun_period} | 先行线：{senkou_period}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由一目均衡图策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
