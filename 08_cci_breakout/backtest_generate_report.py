...
def render_html(results, stats, chart_data, config):
    ...
    cci_n = config.get("cci_n", "N/A")
    level1 = config.get("level1", "N/A")
    level2 = config.get("level2", "N/A")
...
<div class="header">
  <h1>📊 CCI突破策略 — 多品种回测分析报告</h1>
  <div class="subtitle">CCI Breakout Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 CCI周期：{cci_n} | 一级阈值：±{level1} | 二级阈值：±{level2}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由CCI突破策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
