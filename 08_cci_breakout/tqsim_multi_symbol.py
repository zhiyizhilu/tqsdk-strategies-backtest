...
使用方法：
    ...
    4. 可选：运行特定品种：python tqsim_multi_symbol.py --symbol KQ.m@SHFE.ru
"""
...
def run_backtest(symbol: str, config: dict, tq_account: str, tq_password: str) -> dict:
    from strategy import CCIBreakoutStrategy
    ...
    cci_n = config.get("cci_n", 14)
    level1 = config.get("level1", 100)
    level2 = config.get("level2", 200)
    kline_dur = config.get("kline_dur", 3600)
    volume = config.get("volume", 1)
    data_length = config.get("data_length", 200)
    ...
    print(f"CCI周期: {cci_n} | 一级阈值: ±{level1} | 二级阈值: ±{level2}")
...
def main():
    parser = argparse.ArgumentParser(description='CCI突破策略多品种回测')
    parser.add_argument('--symbol', type=str, help='指定测试的品种代码，如 KQ.m@SHFE.ru')
