...
def render_html(results, stats, chart_data, config):
    ...
    n_days = config.get("n_days", "N/A")
    k1 = config.get("k1", "N/A")
    k2 = config.get("k2", "N/A")
...
<title>Dual Thrust策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 Dual Thrust策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Dual Thrust Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 策略参数：N={n_days} | K1={k1} | K2={k2}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由Dual Thrust策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
