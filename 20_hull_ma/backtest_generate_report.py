...
def render_html(results, stats, chart_data, config):
    ...
    hma_period = config.get("hma_period", "N/A")
...
<title>Hull MA策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 Hull MA策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Hull Moving Average Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 HMA周期：{hma_period}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由Hull MA策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
