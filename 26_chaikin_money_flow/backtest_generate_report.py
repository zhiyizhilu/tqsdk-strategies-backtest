...
def render_html(results, stats, chart_data, config):
    ...
    cmf_n = config.get("cmf_n", 20)
    bull_th = config.get("bull_th", 0.05)
    bear_th = config.get("bear_th", -0.05)
...
<title>蔡金资金流量策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 蔡金资金流量策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Chaikin Money Flow Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 CMF窗口：{cmf_n} | 阈值：{bear_th}/{bull_th}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由蔡金资金流量策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
