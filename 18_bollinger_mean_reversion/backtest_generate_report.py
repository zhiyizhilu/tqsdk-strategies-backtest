...
<title>布林带均值回归策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 布林带均值回归策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Bollinger Band Mean Reversion Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 BB({bb_period},{bb_std}) | RSI({rsi_period},{rsi_oversold}/{rsi_overbought})</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由布林带均值回归策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
