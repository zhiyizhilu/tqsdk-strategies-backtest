...
def run_backtest(symbol: str, config: dict, tq_account: str, tq_password: str) -> dict:
    from strategy import PivotPointStrategy
    ...
    day_dur = config.get("day_dur", 86400)
    trade_dur = config.get("trade_dur", 300)
    touch_range = config.get("touch_range", 5.0)
    volume = config.get("volume", 1)
    data_length = config.get("data_length", 50)
    trade_length = config.get("trade_length", 500)
    ...
    print(f"日线: {day_dur}s | 交易周期: {trade_dur}s | 触及范围: {touch_range}点")
...
def main():
    parser = argparse.ArgumentParser(description='枢轴点策略多品种回测')
