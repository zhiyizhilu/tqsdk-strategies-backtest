...
def render_html(results, stats, chart_data, config):
    ...
    trade_dur = config.get("trade_dur", 300)
    touch_range = config.get("touch_range", 5.0)
    trade_name = {60: "1分钟", 300: "5分钟", 900: "15分钟", 1800: "30分钟", 3600: "1小时", 86400: "日线"}.get(trade_dur, f"{trade_dur}秒")
...
<div class="header">
  <h1>📊 枢轴点策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Pivot Point Support &amp; Resistance Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ 交易周期：{trade_name}</span>
    <span>📐 触及范围：{touch_range}点</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由枢轴点策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
