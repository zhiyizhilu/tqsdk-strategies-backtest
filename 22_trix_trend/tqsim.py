...
运行方式：
    ...
    # 指定品种
    python tqsim.py --symbol KQ.m@SHFE.au
...
from strategy import TRIXStrategy


# ===================== 回测配置 =====================
SYMBOL = "KQ.m@SHFE.au"          # 交易合约：连续主力合约
TRIX_PERIOD = 12                  # TRIX三重EMA计算周期
SIGNAL_PERIOD = 9                 # 信号线平均周期
KLINE_DUR = 30 * 60              # K线周期：1800秒 = 30分钟K线
VOLUME = 1                       # 固定持仓手数（如需动态仓位设为 None）
DATA_LENGTH = 500                 # 历史K线数量
...
def run_single_backtest(symbol: str, tq_account: str, tq_password: str):
    ...
    print(f"TRIX周期: {trix_period} | 信号线周期: {signal_period}")
