...
def render_html(results, stats, chart_data, config):
    ...
    trix_period = config.get("trix_period", "N/A")
    signal_period = config.get("signal_period", "N/A")
...
<div class="header">
  <h1>📊 TRIX策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Triple Exponential Average Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 TRIX周期：{trix_period} | 信号线：MA{signal_period}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由TRIX策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
