...
# -*- coding: utf-8 -*-
"""
TRIX三重指数平均策略 (Triple Exponential Average Strategy)
==========================================================

策略逻辑：
    - 计算三重EMA（TRIX指标）及其信号线
    - TRIX上穿信号线且在零轴上方：做多
    - TRIX下穿信号线且在零轴下方：做空
    - TRIX上穿零轴：中期多头趋势确立，做多
    - TRIX下穿零轴：中期空头趋势确立，做空
    - 使用 TargetPosTask 管理持仓，无需手动处理追单、撤单、部分成交等细节

适用品种：
    趋势性较强的品种，如黄金（SHFE.au）、原油（INE.sc）、铜（SHFE.cu）等

风险提示：
    - TRIX三重平滑导致极度滞后，趋势初期信号出现很晚
    - 不适合短线和日内交易，周期过短时信号无意义
    - 在震荡市场中，零轴频繁穿越会产生误导信号
    - 本代码仅供学习参考，不构成任何投资建议
...
class TRIXStrategy:
    ...
    使用方式：
        strategy = TRIXStrategy(api, logger, symbol="SHFE.au2606", trix_period=12, signal_period=9)
        strategy.run()  # 阻塞运行，直到策略结束
    """

    DEFAULT_SYMBOL = "SHFE.au2606"
    DEFAULT_TRIX_PERIOD = 12
    DEFAULT_SIGNAL_PERIOD = 9
    DEFAULT_KLINE_DUR = 30 * 60
    DEFAULT_VOLUME = 1
    ...
    def __init__(
        ...
        self.trix_period = trix_period or self.DEFAULT_TRIX_PERIOD
        self.signal_period = signal_period or self.DEFAULT_SIGNAL_PERIOD
        ...
        self.logger.info(
            f"[策略初始化] TRIX策略 | 合约: {self.symbol} | "
            f"TRIX周期: {self.trix_period} | 信号线周期: {self.signal_period} | "
    ...
    def on_kline_update(self) -> int:
        trix, signal = self.calc_trix(self.klines.close, self.trix_period, self.signal_period)

        trix_now = trix.iloc[-1]
        signal_now = signal.iloc[-1]
        trix_prev = trix.iloc[-2]

        is_cross_up = crossup(trix, signal).iloc[-1]
        is_cross_down = crossdown(trix, signal).iloc[-1]

        trix_cross_zero_up = (trix_prev < 0) and (trix_now >= 0)
        trix_cross_zero_down = (trix_prev > 0) and (trix_now <= 0)

        target_volume = 0

        if is_cross_up and trix_now > 0:
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(f">>> TRIX上穿Signal且在零轴上方！目标仓位: +{dynamic_vol}（做多）")
            target_volume = dynamic_vol
            self.last_signal = "trix_cross_up_above_zero"

        elif trix_cross_zero_up:
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(f">>> TRIX上穿零轴！目标仓位: +{dynamic_vol}（做多）")
            target_volume = dynamic_vol
            self.last_signal = "trix_cross_zero_up"

        elif is_cross_down and trix_now < 0:
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(f">>> TRIX下穿Signal且在零轴下方！目标仓位: -{dynamic_vol}（做空）")
            target_volume = -dynamic_vol
            self.last_signal = "trix_cross_down_below_zero"

        elif trix_cross_zero_down:
            dynamic_vol = self._calc_dynamic_volume()
            self.logger.info(f">>> TRIX下穿零轴！目标仓位: -{dynamic_vol}（做空）")
            target_volume = -dynamic_vol
            self.last_signal = "trix_cross_zero_down"

        elif is_cross_down:
            self.logger.info(f">>> TRIX下穿Signal，平多仓")
            target_volume = 0
            self.last_signal = "trix_cross_down_flat"

        elif is_cross_up:
            self.logger.info(f">>> TRIX上穿Signal，平空仓")
            target_volume = 0
            self.last_signal = "trix_cross_up_flat"
