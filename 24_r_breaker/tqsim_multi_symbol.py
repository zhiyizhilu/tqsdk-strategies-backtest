...
def run_backtest(symbol: str, config: dict, tq_account: str, tq_password: str) -> dict:
    ...
    day_duration = config.get("day_duration", 86400)
    ...
    data_length = config.get("data_length", 10)
    trade_length = config.get("trade_length", 300)
...
def main():
    parser = argparse.ArgumentParser(description='R-Breaker策略多品种回测')
