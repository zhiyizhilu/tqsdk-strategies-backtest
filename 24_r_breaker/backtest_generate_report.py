...
def render_html(results, stats, chart_data, config):
    ...
    trade_duration = config.get("trade_duration", 300)
    close_hour = config.get("close_hour", 14)
    close_minute = config.get("close_minute", 55)
    duration_name = {60: "1分钟", 300: "5分钟", 900: "15分钟", 1800: "30分钟", 3600: "1小时", 86400: "日线"}.get(trade_duration, f"{trade_duration}秒")
...
<title>R-Breaker策略 - 多品种回测分析报告</title>
...
<div class="header">
  <h1>📊 R-Breaker策略 — 多品种回测分析报告</h1>
  <div class="subtitle">R-Breaker Intraday Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{duration_name}</span>
    <span>🕐 强平时间：{close_hour}:{close_minute:02d}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由R-Breaker策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
