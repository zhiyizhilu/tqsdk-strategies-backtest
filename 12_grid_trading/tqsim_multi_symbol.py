...
使用方法：
    ...
    4. 可选：运行特定品种：python tqsim_multi_symbol.py --symbol KQ.m@CZCE.MA
"""
...
def run_backtest(symbol: str, config: dict, tq_account: str, tq_password: str) -> dict:
    ...
    grid_low = config.get("grid_low", 2200)
    grid_high = config.get("grid_high", 2600)
    grid_step = config.get("grid_step", 40)
    volume = config.get("volume", 1)
    max_grid_position = config.get("max_grid_position", 10)
    ...
    initial_balance_val = account_summary['static_balance']
    final_balance = account_summary['balance']
    profit = final_balance - initial_balance_val
    return_rate = (profit / initial_balance_val) * 100 if initial_balance_val > 0 else 0

    print(f"\n{'='*60}")
    print("回测结果")
    print(f"{'='*60}")
    print(f"初始资金: {initial_balance_val:.2f}")
...
def main():
    parser = argparse.ArgumentParser(description='网格交易策略多品种回测')
    parser.add_argument('--symbol', type=str, help='指定测试的品种代码，如 KQ.m@CZCE.MA')
