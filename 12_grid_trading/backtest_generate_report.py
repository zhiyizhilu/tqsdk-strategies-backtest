...
def render_html(results, stats, chart_data, config):
    ...
    grid_low = config.get("grid_low", "N/A")
    grid_high = config.get("grid_high", "N/A")
    grid_step = config.get("grid_step", "N/A")
...
<div class="header">
  <h1>📊 网格交易策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Grid Trading Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ 网格区间：[{grid_low}, {grid_high}] | 间距：{grid_step}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由网格交易策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
