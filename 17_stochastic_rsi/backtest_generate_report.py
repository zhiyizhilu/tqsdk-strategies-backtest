...
<div class="header">
  <h1>📊 随机RSI策略 — 多品种回测分析报告</h1>
  <div class="subtitle">Stochastic RSI Strategy Backtest Report</div>
  <div class="meta">
    <span>📅 回测区间：{start_date} ~ {end_date}</span>
    <span>⚡ K线周期：{kline_name}</span>
    <span>📈 RSI={rsi_period} | Stoch={stoch_period} | K={smooth_k} | D={smooth_d}</span>
    <span>🔲 超买={overbought} | 超卖={oversold}</span>
    <span>🕐 生成时间：{now}</span>
  </div>
</div>
...
<div class="footer">
  本报告由随机RSI策略回测系统自动生成 · 数据仅供参考，不构成投资建议
</div>
